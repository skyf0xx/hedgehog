...
echo "npm's publish step silently drops any file literally named .gitignore"
echo "from the tarball (even when the containing dir is in package.json's"
echo "'files'), so it's stored under this name and bin/cli.mjs renames it"
echo "back to .gitignore on install — see DOTFILE_RENAMES there."
echo ""
